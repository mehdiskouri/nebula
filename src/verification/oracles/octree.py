...
(ARCHITECTURE.md §III.2; Decision #8). The structure whose O(n_active . log n_total)
scaling V0.4 verifies, and which Phase 0 reuses as LOD / multigrid / FMM far-field.

Build: normalize points to [0,1)^3, compute 3-D Morton (Z-order) codes, sort, and
build a linearized (flat-array) octree by recursively partitioning the sorted range
into 8 octants. Each node carries a COARSE PROXY = aggregate mass + center-of-mass
(the Barnes-Hut monopole) computed bottom-up. The early-termination ("opening")
criterion size/distance < theta is what turns the proxy into the log factor: a
distant query stops descending once a node's reduced model is accurate enough. That
soundness rests on the proxy being a valid bounded-error reduced model (V0.1's
premise, one domain over).

Pure numpy. The same flat arrays drive the GPU traversal in octree_gpu.py.
"""
...
@dataclass
class Octree:
    points: np.ndarray     # (n,3) morton-sorted points (in original units)
    masses: np.ndarray     # (n,)  morton-sorted masses
    level: np.ndarray      # (n_nodes,) node depth
    lo: np.ndarray         # (n_nodes,) start index into sorted points
    hi: np.ndarray         # (n_nodes,) end index (exclusive)
    center: np.ndarray     # (n_nodes,3) geometric cell center (normalized coords)
    half: np.ndarray       # (n_nodes,) half side length (normalized)
    child: np.ndarray      # (n_nodes,8) child node index or -1
    is_leaf: np.ndarray    # (n_nodes,) bool
    mass: np.ndarray       # (n_nodes,) aggregate proxy mass
    com: np.ndarray        # (n_nodes,3) aggregate center-of-mass (original units)
    size: np.ndarray       # (n_nodes,) cell side length in ORIGINAL units (for MAC)
    n_nodes: int
...
def build(points, masses, leaf_capacity=16, max_level=MORTON_BITS):
    ...
    unit = (points - lo_box) / (span * (1 + 1e-9))            # into [0,1)
    codes = morton_codes(unit)
    order = np.argsort(codes, kind="stable")
    pts = points[order]; mas = masses[order]; codes = codes[order]

    # flat node arrays (python lists, grown via an explicit work stack)
    L_, LO_, HI_, CX_, HALF_, LEAF_ = [], [], [], [], [], []
    CHILD_ = []
    ...
    while stack:
        ...
        # sorted -> octants are non-decreasing; split by searchsorted
    ...
    size = 2.0 * half * span                                  # original-unit side length

    # bottom-up aggregation: children always have higher index than parent
    mass = np.zeros(n_nodes); com = np.zeros((n_nodes, 3))
    csum = np.cumsum(np.concatenate([[0.0], mas]))            # prefix sums for leaf ranges
...
class LinTree:
    ...
    In preorder a subtree occupies a contiguous index block, so the first child is
    `idx+1` and `escape[idx] = idx + subtree_size` (where to jump when a node is
    accepted/skipped). The same arrays drive the CPU and the Warp GPU traversal,
    guaranteeing identical logic.
    """
...
def dfs_linearize(tree):
    ...
    stack = [0]; k = 0
    while stack:                                  # preorder DFS; push children reversed
        ...
        if not tree.is_leaf[nd]:
            for o in range(7, -1, -1):
                ...
                if ci >= 0:
                    stack.append(ci)
    # subtree sizes in new (preorder) indexing: children occupy higher indices
    ...
    for newidx in range(n - 1, 0, -1):            # accumulate child sizes into parents
...
def bh_field(lin, queries, theta=0.5, eps=1e-4, G=1.0):
    ...
    Accept a node's monopole when size/distance < theta (early termination) and jump
    to escape[node]; otherwise descend (idx+1); leaves summed directly. Returns
    (phi, interactions, depth): `interactions` = exact work/query (node + point evals),
    `depth` = max node level visited/query.
    """
    ...
    for i in range(nq):
        ...
        while idx < lin.n_nodes:
            ...
            elif size[idx] / d < theta:                       # MAC -> accept monopole
                p += G * mass[idx] / d; cnt += 1; idx = escape[idx]
            else:
                idx += 1                                      # descend to first child
...
if __name__ == "__main__":
    ...
    print(f"1) build: n={n}  nodes={tree.n_nodes}  leaves={int(tree.is_leaf.sum())}  "
          f"max level={tree.level.max()}")
    # invariants: every point in exactly one leaf; root mass = total mass; root com correct
    leaf_pts = sum(int(tree.hi[i] - tree.lo[i]) for i in range(tree.n_nodes) if tree.is_leaf[i])
    print(f"   points covered by leaves = {leaf_pts} (== {n})")
    print(f"   root mass err = {abs(tree.mass[0]-mas.sum()):.2e}   "
          f"root com err = {np.abs(tree.com[0]-(pts*mas[:,None]).sum(0)/mas.sum()).max():.2e}")

    # correctness: BH vs direct over a theta sweep (proxy/early-termination error bounded)
    lin = dfs_linearize(tree)
    print(f"   dfs-linearized: escape[root]={lin.escape[0]} (== n_nodes {tree.n_nodes})")
    q = rng.random((200, 3))
    exact = direct_field(pts, mas, q)
    ...
    for theta in (1.0, 0.7, 0.5, 0.3):
        ...
        print(f"   theta={theta}: mean relerr={relerr.mean():.2e}  max={relerr.max():.2e}  "
              f"interactions/q={inter.mean():.1f}  mean depth={depth.mean():.1f}")
