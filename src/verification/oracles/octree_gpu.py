...
One thread per query point walks the DFS-linearized node arrays (octree.LinTree)
stackless via escape pointers — identical logic to octree.bh_field, but on the GPU
(ARCHITECTURE Part V: Taichi/Warp substrate). Used by V0.4 to push n to 10^6+ and
corroborate the O(n_active . log n_total) scaling in wall-clock at Phase-0 scale.
"""
import numpy as np
...
if _HAS_WARP:
    ...
    def _bh_count_kernel(com: wp.array(dtype=wp.vec3), size: wp.array(dtype=wp.float32),
        ...
        """Counts interactions (node + point evals) and max descent depth per query."""
        i = wp.tid()
        q = queries[i]
...
if __name__ == "__main__":
    ...
    # 1) GPU field matches CPU bh_field and the direct oracle.
    ...
    # 2) wall-clock at increasing n (full activation: queries = points)
    ...
    for nn in (50000, 200000, 1000000):
        ...
        bh_field_gpu(dv, Q[:1000], theta=0.5)            # warm up
        t0 = time.time(); bh_field_gpu(dv, Q, theta=0.5); dt = time.time() - t0
        print(f"   n={nn:>8d}  nodes={tr.n_nodes:>8d}  query_time={dt:.3f}s  "
              f"({nn/dt/1e6:.2f}M queries/s)")
