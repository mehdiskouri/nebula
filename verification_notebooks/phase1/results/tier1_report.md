...
**Status: TIER 1 COMPLETE — all nine verifications PASS (V1.1–V1.9).** All Tier-1 phase gates are met: the Phase-0 gate (V1.1–V1.3) is COMPLETE; V1.4 unblocks the authoring/spectral phase; V1.5 + V1.6 unblock living-asset work; V1.7 validates the morphogenetic-scaffold mechanism (Decision #22); V1.8 validates the growth-trace memoization + write-back/heal loop (Decision #11); V1.9 validates the dual-cloud render economy (Decision #6). Every Tier-1 subsystem mechanism is now verified.
...
| V1.7 | Skeleton precipitation under load | stress-driven Wolff's-law deposition self-organizes a connected, load-bearing skeleton competitive with the SIMP topology-optimization optimum, and the skeleton responds to load (shrinks at low gravity; precipitates near-nothing under a radiance support law-domain) | ✅ PASS |
| V1.8 | Growth memoization & write-back | a memoized growth trace evaluates bit-exactly to a fresh grow at any (time, LOD), and a write-back (recorded cut) invalidates stale sub-results so growth heals around the wound; omitting the write-back from the cache key produces stale replays (proving the key must include it) | ✅ PASS |
| V1.9 | Dual-cloud skinning fidelity | a dense render cloud skinned (rotation-aware LBS) to a coarse physics cloud reproduces the full-resolution deformation within tolerance and gracefully, with reduction-independent error into the thousands× (GPU skins >1.5M render points in ms); translation-only skinning collapses (candy-wrapper), proving per-node rotation is required | ✅ PASS |
...
**Shared oracle modules** (`src/verification/oracles/`): `fire_operators.py`, `bus_runtime.py`, `monolithic_fire.py`, `multirate.py`, **`semi_implicit_fire.py`** (new for V1.2), **`jensen_rate.py`** (new for V1.3), **`regulator.py`** (new for V1.5 — the regulated cardiovascular loop + basin/separatrix/viability-margin oracle), **`regulator_stability.py`** (new for V1.6 — the inertial-actuator model + linear-stability/limit-cycle oracle for the passivity claim), **`topology_opt.py`** (new for V1.7 — the 3D linear-elastic BVP FE solver + cantilever/creature load-case domains + the SIMP topology-optimization oracle), **`wolff.py`** (new for V1.7 — the Wolff's-law fully-stressed remodeling mechanism under test), **`growth.py`** (new for V1.8 — the field-biased L-system growth front + memoizable trace + write-back/heal, with the from-scratch grow oracle), **`dualcloud.py`** (new for V1.9 — coarse physics cloud + rotation-aware LBS of a dense render cloud + translation foil, numpy + Warp GPU, vs an exact deformation-field oracle), `transitions.py`, `determinism.py`. **Coupling-pipeline modules** (`src/verification/`): **`symmetry_basis.py`** (new for V1.4 — the factored §8 symmetry core + regression self-check) and **`coupling_pipeline.py`** (new for V1.4 — the geometric pipeline), built on the untouched §8 baseline scripts `coupling_operator_core.py` (Z₂) and `coupling_operator_c6.py` (C₆).
...
**Targets:** Decision #22; ARCHITECTURE §III.7 ("the morphogenetic scaffold — the skeleton is not the origin, it is a *precipitate*"). **Depends on** — (none). First Tier-1 verification that is **throughput-bound and genuinely GPU-using** (3D FE via cupy-CG), unlike CPU-bound V1.1–V1.6.

**The mechanism, stated precisely.** Wolff's law — bone condenses under stress, the same rule a tree uses for reaction wood — is implemented as Huiskes/Mullender homeostatic remodeling in its **fully-stressed form**: seed a full block, sense the spatially-filtered strain-energy stimulus `S = (sensor filter)·uₑᵀKe0uₑ`, and remodel toward a homeostatic setpoint `k` by the **multiplicative update `ρ ← ρ·(S/k)^η`** (η≈0.3), move-limited, clipped to [ρ_min,1]. The multiplicative form is the load-bearing fix and a genuine finding: the stimulus is steeply density-dependent (`S∝ρ⁻⁴` for a loaded strut), so a *linear* `ρ += rate·(S−k)` rule is a stiff feedback that **oscillates and fragments the structure** (observed directly in calibration: largest-component fraction stalls at ~0.5, never converges); the FSD update is matched to that exponent and converges (≈30–105 iterations). The Mullender sensor radius is the mesh-independence regularizer; the resorb-from-full start keeps the load path connected (BESO-like).

**Independent oracle.** Standard **SIMP** compliance minimization (OC update + Sigmund sensitivity filter) — a mature, trusted global optimizer — on the *same* 3D linear-elastic boundary-value FE solver, so the comparison is physics-identical.

**Implementation.**
- **`topology_opt.py` (new)** — the 3D FE **boundary-value** solver (`dns_elasticity_3d.py` is *periodic homogenization*, not a BVP), the cantilever + creature load-case domains, and the SIMP oracle. Reuses `element_stiffness` (the H8 element, `_NAT` convention) from `dns_elasticity_3d.py` and `isotropic_stiffness` from `homogenization.py`; cupy-CG solve with CPU sparse-LU fallback; CG tolerance regime (V0.5). Loads are design-**independent** (cantilever: clamp + tip load; creature: feet fixed + a fixed nominal tissue-mass gravity body load scaled by `g·(1−support_alpha)`), so the matched-volume compliance comparison is exact.
- **`wolff.py` (new)** — the FSD remodeling mechanism under test; imports the FE solver/domains from `topology_opt.py` (no new physics).
- Notebook `V1_7_skeleton_precipitation.ipynb`; figure `V1_7_skeleton_precipitation.png`.

**Verification.** Pre-registered criteria (frozen with margin in `_calib_v17.py`): **(A,B) connectivity** — the largest 26-connected component of the `ρ>0.5` solid set holds **≥0.95** of the material and reaches the supports, for both the cantilever and the creature Earth-g skeleton; **(A,B) compliance** — *continuous* `C_wolff/C_simp ≤ 2.0` at matched continuous volume (`volfrac=mean(ρ_w)`, both scored with the same penalized stiffness ⇒ ratio ≥ 1; comparing binarized designs would inject a thresholding-disconnection and is avoided); **(C) morphology monotone** — creature solid-fraction strictly decreases as gravity decreases; **(C) seraph near-nothing** — a radiance support law-domain (α=1, Decision #26) settles solid-fraction **≤0.02**; **(D) determinism** — repeat-run compliance rel. diff **<1e-6**.

**Results.**
- **A — cantilever (calibration):** Wolff precipitates a single connected load-path (component fraction **1.000**, reaches the clamp); compliance ratio **1.172** vs the SIMP optimum at matched volume (71 iterations).
- **B — creature Earth-g:** a connected, load-bearing skeleton precipitates from the full block (fraction **1.000**, reaches the feet); compliance ratio **1.029** — essentially optimal.
- **C — morphology & seraph:** solid-fraction **0.855 → 0.624 → 0.208** as gravity drops (g = 1.0 → 0.4 → 0.15) — cleanly monotone; the seraph support-field (gravity cancelled) settles **0.0000** (near-nothing). (The g=0.4 run did not hit the strict convergence tolerance but its final morphology is unambiguous; at very low load the sparse precipitate naturally thins toward nothing — exactly the architecture's prediction.)
- **D — determinism:** repeat-run compliance rel. diff **0.00e+00**.

**Verdict: PASS.** Stress-driven deposition (Wolff's law) self-organizes a connected, load-bearing skeleton competitive with the global SIMP optimum, and the skeleton responds correctly to the load — shrinking under low gravity and precipitating near-nothing under a radiance support law-domain. **Decision #22 holds: the skeleton is a precipitate, not an authored atlas.** **Standing note (the deposition operator):** the fully-stressed *multiplicative* update is required — a naive linear SED rule is numerically unstable (oscillates and fragments). This is wired as the precipitation rule itself, not new machinery. V1.7 gates its own subsystem (skeleton/morphogenesis) and is not a phase exit-criterion.

---

## V1.8 — Growth memoization & write-back correctness

**Targets:** Decision #11; ARCHITECTURE §III.1 ("Growth — writing the implicit field": growth is an active front depositing ringed layers, memoized into a compact trace evaluated at any (time, LOD), and *not only additive* — wounds heal by write-back). **Depends on** V0.5 (determinism). CPU/numpy (tiny seeded trees, latency-bound) — back to CPU-bound after V1.7's GPU.

**The mechanism, stated precisely.** A dimension-agnostic field-biased L-system front (apical meristems sense a light field, branch by a seeded rule, deposit ringed wood layers). **The load-bearing design point:** every branch's RNG stream is derived by **hashing its key** (a stable `blake2b` digest of its lineage path), *not* the global draw order — so a subtree is a **pure deterministic function of `(seed, params, key, time, LOD, write-back state)`**. That is exactly what makes per-subtree memoization sound and bit-reproducible (a memoized subtree reproduces the fresh one to the bit); builtin `hash()` is salted per process and would silently break it (Decision #3 / V0.5 — "the program IS the asset"). LOD = branch-order truncation ("truncation IS level-of-detail"); a write-back (recorded cut) severs the distal limb and a **callus healing front** deposits over the wound — growth reading the write-back.

**Independent oracle.** A from-scratch, non-memoized grow (intact, and a grow-with-the-cut for the healed tree).

**Implementation.**
- **`growth.py` (new)** — the growth front + memoizable trace + `(time,LOD)` evaluator + subtree memo cache (with an **include/omit write-back-in-key toggle** to demonstrate necessity) + `apply`-cut/callus healing + the from-scratch oracles. Reuses `determinism.bitwise_equal` for the bit-exact position checks and `determinism`'s integer-exact / canonical-order hashing discipline for the write-back-state cache key.
- Notebook `V1_8_growth_writeback.ipynb`; figure `V1_8_growth_writeback.png`.

**Verification.** Pre-registered criteria (frozen with margin in `_calib_v18.py`, robustness-checked across 5 seeds × both dims × single/multi-cut scenarios): **(A1)** memoized-trace evaluation == fresh grow, **bit-exact** for 100% of `(time,LOD)` samples in both 2D & 3D (tol = 0); **(A2)** repeated grow bit-identical; **(B1)** with the write-back in the cache key → **0 stale-replay incidents** and healed == oracle (bit-exact); **(B2, necessity)** with the write-back **omitted** → stale-replay incidents **≥ 1**; **(B3, healing)** callus deposited over the wound **> 0** and the severed limb pruned (absent) — the necessity demonstration mirrors V1.1's "ambiguous without the cascade."

**Results.**
- **A — memo == fresh:** **115/115** `(time,LOD)` samples bit-exact in **both** 2D and 3D (0 mismatches); repeated-grow digests equal (determinism holds via the stable hashed sub-seeds).
- **B — write-back:** with the write-back in the key, **0** stale replays and the healed tree equals the from-scratch-with-cut oracle bit-for-bit (callus **16** [2D] / **24** [3D] nodes, severed limb pruned); with the write-back **omitted**, the stale intact subtree is served — **150** [2D] / **119** [3D] stale-replay nodes (the severed limb reappears). The key *must* include the write-back state.

**Verdict: PASS.** A memoized growth trace evaluates bit-exactly to a fresh grow at any (time, LOD), and a write-back correctly invalidates stale sub-results so growth heals around the wound rather than replaying the pre-wound limb. **Decision #11 holds.** **Standing requirement (the cache contract):** the memoization key must include the write-back state — omitting it produces stale replays, demonstrated directly. V1.8 gates its own subsystem (growth) and is not a phase exit-criterion. Notebook `V1_8_growth_writeback.ipynb`; figure `V1_8_growth_writeback.png`.

---

## V1.9 — Dual-cloud skinning fidelity

**Targets:** Decision #6; ARCHITECTURE §III.1 ("Rendering is decoupled: the dual cloud" — a coarse physics cloud drives a dense render cloud of Gaussian splats riding the deformation via skinning weights). **Depends on** none. The final Tier-1 verification; uses the GPU at full scale (like V1.7).

**The mechanism, stated precisely.** Rotation-aware **linear-blend skinning** `p = Σ wᵢ (Rᵢ (p₀−Xᵢ) + xᵢ)`, where each coarse node's rotation `Rᵢ` is estimated by polar decomposition (shape matching) from the coarse cloud's *own* neighbour graph — the shippable system does not know the true deformation. The **translation-only foil** `p = p₀ + Σ wᵢ (xᵢ−Xᵢ)` (no per-node rotation) is the contrast: the candy-wrapper collapse. The load-bearing finding is that **rotation-aware LBS error is reduction-independent** (it reconstructs the local rigid frame), so the physics cloud can be made arbitrarily coarse — which is *why* the dual-cloud economy works.

**Independent oracle.** An **exact continuum deformation field** φ (pure bending into a constant-curvature arc, helical twist, stretch) — the true deformed position of *every* render point at *any* resolution. This is the strongest oracle for a skinning test: zero solver noise, fully controllable severity, and it makes the ground truth available for skinning of **millions** of render points on the GPU. (Physics-solver fidelity is a separate concern, already validated by V1.7's FE work; isolating the skinning operator is deliberate. An XPBD lattice oracle was prototyped first but, under-resolved/un-scaled across resolutions, it conflated coarsening-physics error with skinning error — the exact field cleanly isolates the claim under test.)

**Implementation.**
- **`dualcloud.py` (new)** — the beam lattices, the exact `deform_field` oracle, per-node polar-decomposition rotations, rotation-aware LBS + the translation foil, the `run_case` harness, and a **Warp GPU LBS kernel** (`skin_lbs_gpu`) that skins the dense cloud at full scale on the RTX 4090. Reuses the `_HAS_WARP` pattern (`octree_gpu`/`determinism`) and `determinism.rel_diff` for the GPU↔numpy cross-check.
- Notebook `V1_9_dualcloud_skinning.ipynb`; figure `V1_9_dualcloud_skinning.png`.

**Verification.** Pre-registered criteria (frozen with margin in `_calib_v19.py`): **(A1)** rotation-aware LBS mean error ≤ **0.01** of beam length at the max deformation (bend 90°, twist 180°), ≥10× reduction; **(A2)** error monotone with max step-to-step jump < **0.005** (no popping); **(B)** translation-only error ≥ **2×** worse under bend & twist (candy-wrapper necessity, mirroring V1.1/V1.8's foils); **(C1)** GPU == numpy within rel diff < **1e-4**; **(C2)** the GPU path skins ≥ **10⁵** render points at ≥ **100×** reduction with LBS mean ≤ 0.01.

**Results.**
- **A — fidelity & graceful:** at 75× reduction, LBS mean error **0.0018** (bend 90°) / **0.0035** (twist 180°) — well under 1%; sweeps are monotone with max step-jump **0.0002 / 0.0004** (no popping).
- **B — necessity:** the translation foil is **6.7×** (bend) / **7.8×** (twist) worse — the cross-sections visibly collapse (candy-wrapper) while rotation-aware LBS rides the twist.
- **C — reduction-independence + scale:** LBS error stays **~0.0034 flat from 6× to 954×** reduction (the foil grows to 8×+ worse); GPU == numpy at **1.2e-5**; the GPU skins **1,540,081** render points at **7404×** reduction in **0.049 s**, error **0.0034** — "simulate hundreds, render millions" demonstrated literally.

**Verdict: PASS.** A dense render cloud skinned (rotation-aware LBS) to a coarse physics cloud reproduces the full-resolution deformation within tolerance, gracefully, at reduction ratios into the thousands, and the GPU skins >1.5M render points in milliseconds. **Decision #6 holds.** **Standing note (the skinning contract):** per-node rotation is required — translation-only skinning collapses (candy-wrapper); this is the shippable deformation operator, not new machinery. The CONSTRAIN fallback (cap deformation per coarse resolution) was not needed in the tested regime. Notebook `V1_9_dualcloud_skinning.ipynb`; figure `V1_9_dualcloud_skinning.png`.

---

## Tier-1 status & remaining work
**Tier 1 is COMPLETE — all nine verifications PASS (V1.1–V1.9).** The §9 exit criteria are met for every downstream phase gated by Tier 1: the **Phase-0 tree slice** (Tier 0 + V1.1–V1.3 — substrate, homogenization currency, conservation/composition, scaling, determinism, order-free composition, stiff-split stability, nonlinear-rate lumping), the **authoring/spectral phase** (V1.4, on the proven symmetry core), and **living-asset work / Phase 2** (V1.5 regulated homeostasis + reserve-dependent mortality, V1.6 passivity stability). **V1.7 (skeleton precipitation) passes**, validating the morphogenetic-scaffold mechanism (Decision #22); **V1.8 (growth memoization & write-back) passes**, validating the growth-trace memoization + heal-on-write-back loop (Decision #11); **V1.9 (dual-cloud skinning fidelity) passes**, validating the coarse-physics-drives-dense-render economy (Decision #6). **No Tier-1 verifications remain — Tier 1 is complete.** The natural next front is Tier 2 (the known hard risks: V2.1 RVE↔surrogate handoff, V2.2 percolation, V2.3 geo/physical misalignment, V2.4 surrogate generalization, V2.5 inverse design), which gate the scale & exotic claims.
